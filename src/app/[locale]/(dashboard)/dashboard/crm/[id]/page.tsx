      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            {!!client.is_favorite && <span className="text-xl text-amber-500">★</span>}
              ...
              <div className="space-y-3">
                {notes.map((note: Record<string, unknown>) => (
                  <div
                    ...
                    className={`rounded-lg border p-3 ${note.is_pinned ? 'border-amber-300 bg-amber-50 dark:bg-amber-950/20' : 'border-border'}`}
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <div className="flex items-center gap-2">
                        {!!note.is_pinned && <span className="text-amber-500 text-xs">📌</span>}
            ...
            {(reminders && reminders.length > 0) ? (
              <div className="space-y-2">
                {reminders.map((r: Record<string, unknown>) => (
                  <div
                    key={r.id as string}
                    className={`rounded-lg border p-2 text-xs ${r.is_completed ? 'border-border opacity-50 line-through' : 'border-amber-300'}`}
                  >
