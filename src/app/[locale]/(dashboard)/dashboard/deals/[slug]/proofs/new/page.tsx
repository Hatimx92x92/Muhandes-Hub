...
export default async function NewProofPage({
  ...
  let deal;
  if (isUUID(slug)) {
    const { data } = await db(supabase).from('deals').select('id, buyer_id, seller_id, status, seller_progress, buyer_progress, title_slug').eq('id', slug).single();
    deal = data;
  } else {
    const { data } = await db(supabase).from('deals').select('id, buyer_id, seller_id, status, seller_progress, buyer_progress, title_slug').eq('title_slug', slug).single();
    deal = data;
  }
